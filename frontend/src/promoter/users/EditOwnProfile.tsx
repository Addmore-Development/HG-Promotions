...
// No onboarding flow here. Just view/edit what's already on the account.
...
export const EditOwnProfile: React.FC = () => {
  ...
  const fs: React.CSSProperties = { width:'100%', padding:'12px 14px', background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:'8px', color:'#fff', fontSize:'14px', outline:'none', boxSizing:'border-box' };
  const ls: React.CSSProperties = { display:'block', color:'#a0a0a0', fontSize:'11px', fontWeight:600, letterSpacing:'0.06em', textTransform:'uppercase', marginBottom:'6px' };
  const SH = ({ t }: { t: string }) => <h3 style={{ color:'#D4AF37', fontSize:'11px', fontWeight:700, letterSpacing:'0.08em', textTransform:'uppercase', margin:'24px 0 16px' }}>{t}</h3>;
  ...
  const displayName = profile?.fullName || (user as { name?: string })?.name || 'Promoter';
  ...
  return (
    <div style={{ maxWidth:'640px' }}>
      ...
      {profile?.onboardingStatus === 'rejected' && profile.rejectionReason && (
        <div style={{ padding:'16px', background:'rgba(239,68,68,0.08)', border:'1px solid rgba(239,68,68,0.25)', borderRadius:'12px', marginBottom:'24px' }}>
          <p style={{ color:'#f87171', fontSize:'13px', margin:0 }}>
            <strong>Rejection reason:</strong> {profile.rejectionReason}. Please contact support or update your documents.
          </p>
        </div>
      )}

      {/* Read-only identity card */}
      <div style={{ padding:'24px', background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:'16px', marginBottom:'28px' }}>
        <div style={{ display:'flex', alignItems:'center', gap:'16px', marginBottom:'20px' }}>
          <div style={{ width:'64px', height:'64px', borderRadius:'50%', overflow:'hidden', flexShrink:0, background:'linear-gradient(135deg,#D4AF37,#B8962E)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'24px', fontWeight:900, color:'#0A0A0A', border:'2px solid rgba(212,175,55,0.3)' }}>
          ...
          <div>
            ...
            {profile?.city && profile?.province && (
              <p style={{ color:'#555', fontSize:'12px', margin:0 }}>📍 {profile.city}, {profile.province}</p>
            )}
          </div>
        </div>
        ...
        {/* Physical attributes — read-only display */}
        {profile?.physicalAttributes?.height > 0 && (
          <div style={{ marginTop:'16px', paddingTop:'16px', borderTop:'1px solid rgba(255,255,255,0.06)', display:'flex', gap:'24px', flexWrap:'wrap' }}>
            {[
              ...
              { label:'Shoe Size', value: profile.physicalAttributes.shoeSize || '—' },
            ].map(a => (
              <div key={a.label}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:'0 16px' }}>
        {[{key:'height' as const,label:'Height (cm)',ph:'168'},{key:'clothingSize' as const,label:'Clothing Size',ph:'S/M/L'},{key:'shoeSize' as const,label:'Shoe Size',ph:'7'}].map(f => (
      ...
      <SH t="Social Media" />
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0 16px' }}>
        {[{key:'instagram' as const,label:'Instagram',ph:'@handle'},{key:'tiktok' as const,label:'TikTok',ph:'@handle'}].map(f => (
          <div key={f.key} style={{ marginBottom:'16px' }}>
            <label style={ls}>{f.label}</label>
            <input style={fs} value={form[f.key]} onChange={e => setForm(p => ({...p,[f.key]:e.target.value}))} placeholder={f.ph} />
          </div>
