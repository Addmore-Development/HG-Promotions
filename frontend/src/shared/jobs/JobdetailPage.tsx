// shared/jobs/JobDetailPage.tsx — Updated palette
    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.92)', backdropFilter:'blur(16px)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000, padding:24 }}
      ...
      <div style={{ background:BC2, border:`1px solid rgba(212,136,10,0.28)`, width:'100%', maxWidth:480, position:'relative', overflow:'hidden', borderRadius:4 }}>
        ...
        <div style={{ padding:'36px 40px' }}>
          ...
          <div style={{ width:56, height:56, borderRadius:'50%', background:`rgba(196,151,58,0.12)`, border:`2px solid rgba(196,151,58,0.35)`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:22, marginBottom:20 }}>🚫</div>
          ...
          <h2 style={{ fontFamily:FD, fontSize:22, fontWeight:700, color:W, marginBottom:12, lineHeight:1.3 }}>You need a Promoter account to apply for jobs</h2>
          <p style={{ fontSize:13, color:WM, lineHeight:1.75, marginBottom:28, fontFamily:FB }}>Only registered promoters can apply for shifts on the Honey Group platform. Business accounts and admin accounts are not eligible to apply for jobs.</p>
          <p style={{ fontSize:13, color:WM, lineHeight:1.75, marginBottom:28, fontFamily:FB }}>If you're a promoter, please log in to your promoter account. If you haven't registered yet, create a promoter account — it's free.</p>
          <div style={{ display:'flex', gap:12 }}>
            <button onClick={()=>{ onClose(); onNavigate('/login') }} style={{ flex:1, padding:'13px', background:`linear-gradient(135deg,${G},${GL})`, border:'none', color:B, fontFamily:FB, fontSize:12, fontWeight:700, letterSpacing:'0.1em', textTransform:'uppercase', cursor:'pointer', borderRadius:3, transition:'all 0.2s' }} onMouseEnter={e=>e.currentTarget.style.opacity='0.88'} onMouseLeave={e=>e.currentTarget.style.opacity='1'}>Log In as Promoter</button>
            <button onClick={()=>{ onClose(); onNavigate('/register') }} style={{ flex:1, padding:'13px', background:'transparent', border:`1px solid rgba(196,151,58,0.45)`, color:G, fontFamily:FB, fontSize:12, fontWeight:700, letterSpacing:'0.1em', textTransform:'uppercase', cursor:'pointer', borderRadius:3, transition:'all 0.2s' }} onMouseEnter={e=>{ e.currentTarget.style.background='rgba(196,151,58,0.10)'; e.currentTarget.style.borderColor=G }} onMouseLeave={e=>{ e.currentTarget.style.background='transparent'; e.currentTarget.style.borderColor='rgba(196,151,58,0.45)' }}>Register as Promoter</button>
          </div>
          <p style={{ marginTop:16, fontSize:11, color:WD, textAlign:'center', fontFamily:FB }}>Already a promoter? <button onClick={()=>{ onClose(); onNavigate('/login') }} style={{ background:'none', border:'none', color:G, cursor:'pointer', fontSize:11, fontFamily:FB, textDecoration:'underline' }}>Log in here</button></p>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div style={{ minHeight:'100vh', background:B, fontFamily:FB, color:W }}>
      ...
      <nav style={{ padding:'18px 48px', display:'flex', justifyContent:'space-between', alignItems:'center', borderBottom:`1px solid ${BB}`, background:BC, position:'sticky', top:0, zIndex:50 }}>
        <button onClick={()=>navigate('/jobs')} style={{ background:'none', border:'none', color:WM, cursor:'pointer', fontFamily:FB, fontSize:13, display:'flex', alignItems:'center', gap:6 }} onMouseEnter={e=>e.currentTarget.style.color=W} onMouseLeave={e=>e.currentTarget.style.color=WM}>← Back to Jobs</button>
        <div style={{ fontFamily:FD, fontSize:16, fontWeight:700 }}><span style={{ color:G }}>HONEY</span><span style={{ color:W }}> GROUP</span></div>
        <div>
          {session ? (
            <span style={{ fontSize:12, color:WM, fontFamily:FB }}>Logged in as <strong style={{ color:G }}>{session.name||session.role}</strong></span>
          ) : (
      ...
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'48px 24px 80px', display:'grid', gridTemplateColumns:'1fr 320px', gap:32, alignItems:'start' }}>
        <div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:1, background:BB, marginBottom:28 }}>
            {[{label:'Rate',value:pay,sub:payPer},{label:'Date',value:date,sub:duration},{label:'Slots',value:`${slotsLeft} left`,sub:`of ${slots} total`}].map((r,i)=>(
              <div key={i} style={{ background:BC2, padding:'18px 20px' }}>
                <div style={{ fontSize:9, letterSpacing:'0.2em', textTransform:'uppercase', color:'rgba(192,178,158,0.82)', marginBottom:6 }}>{r.label}</div>
                <div style={{ fontFamily:FD, fontSize:22, fontWeight:700, color:G }}>{r.value}</div>
                {r.sub&&<div style={{ fontSize:11, color:'rgba(192,178,158,0.82)', marginTop:3 }}>{r.sub}</div>}
              </div>
            ))}
          </div>
          {!isOpen&&<div style={{ padding:'12px 18px', background:'rgba(139,90,26,0.18)', border:`1px solid rgba(139,90,26,0.5)`, borderRadius:3, marginBottom:24, fontSize:13, color:'#C8B898' }}>⚠ This job is currently {status} — applications may not be accepted.</div>}
          {tags.length>0&&(
          ...
          <div style={{ marginBottom:24, padding:'20px 24px', background:BC2, border:`1px solid ${BB}`, borderRadius:3 }}>
            ...
            {[['Company',company],['Location',location],['Date',date],['Duration',duration],['Pay',`${pay} ${payPer}`],['Slots Available',`${slotsLeft} of ${slots}`],...(contactPerson?[['Contact Person',contactPerson]]:[]),...(contactPhone?[['Contact Phone',contactPhone]]:[]),...(contactEmail?[['Contact Email',contactEmail]]:[]),...(companyReg?[['Company Reg.',companyReg]]:[])]
        ...
        <div style={{ position:'sticky', top:88, alignSelf:'start' }}>
          <div style={{ background:BC2, border:`1px solid rgba(212,136,10,0.28)`, borderRadius:4, overflow:'hidden' }}>
            ...
            <div style={{ padding:'28px 24px' }}>
              ...
              <div style={{ fontSize:12, color:'rgba(192,178,158,0.82)', marginTop:4, marginBottom:16 }}>{payPer}</div>
              <div style={{ display:'flex', flexDirection:'column', gap:10, marginBottom:20 }}>
                {[{icon:'📅',text:date},{icon:'⏱',text:duration},{icon:'📍',text:location},{icon:'👥',text:`${slotsLeft} slots remaining`}].filter(m=>m.text).map((m,i)=>(
              ...
              <div style={{ marginBottom:20 }}>
                <span style={{ fontSize:9, fontWeight:700, letterSpacing:'0.16em', textTransform:'uppercase', color:isOpen?GL:'rgba(206,197,178,0.40)', background:isOpen?'rgba(232,168,32,0.12)':'rgba(206,197,178,0.05)', border:`1px solid ${isOpen?'rgba(232,168,32,0.35)':'rgba(206,197,178,0.12)'}`, padding:'4px 12px', borderRadius:2 }}>
                  {status}
                </span>
              </div>
              <button onClick={handleApplyClick} disabled={!isOpen}
                style={{ width:'100%', padding:'15px', background:isOpen?`linear-gradient(135deg,${G},${GL})`:'rgba(206,197,178,0.05)', border:'none', color:isOpen?B:WM, fontFamily:FB, fontSize:13, fontWeight:700, cursor:isOpen?'pointer':'not-allowed', letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:10, borderRadius:3, transition:'all 0.2s', boxShadow:isOpen?`0 4px 20px rgba(196,151,58,0.35)`:'none' }}
              ...
              <button onClick={()=>navigate('/jobs')} style={{ width:'100%', padding:'12px', background:'transparent', border:`1px solid ${BB}`, color:WM, fontFamily:FB, fontSize:12, cursor:'pointer', borderRadius:3, transition:'all 0.2s' }}
              ...
              {session && userRole !== 'promoter' && (
                <div style={{ marginTop:14, padding:'10px 14px', background:'rgba(139,90,26,0.15)', border:`1px solid rgba(139,90,26,0.4)`, borderRadius:3, fontSize:11, color:'#C8B898', lineHeight:1.6, fontFamily:FB }}>
