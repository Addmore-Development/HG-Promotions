...
const EMPTY: Omit<User, 'id' | 'jobs' | 'earnings'> = { name: '', email: '', phone: '', role: 'promoter', status: 'active', city: '', joined: '' }
    ...
    <AdminLayout>
      <div style={{ padding: '40px 48px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 32 }}>
          ...
          <button onClick={openCreate} style={{ padding: '12px 28px', background: G, border: 'none', color: B, fontFamily: FB, fontSize: 12, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', cursor: 'pointer', transition: 'background 0.2s' }}
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 24, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            {(['all', 'promoter', 'supervisor', 'admin'] as const).map(r => (
              <button key={r} onClick={() => setRoleF(r)} style={{ padding: '7px 14px', border: 'none', borderRadius: 4, cursor: 'pointer', fontFamily: FB, fontSize: 11, fontWeight: 600, textTransform: 'capitalize', background: roleF === r ? G : 'rgba(255,255,255,0.05)', color: roleF === r ? B : WM, transition: 'all 0.2s' }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            {(['all', 'active', 'inactive', 'suspended'] as const).map(s => (
              <button key={s} onClick={() => setStatusF(s)} style={{ padding: '7px 14px', border: 'none', borderRadius: 4, cursor: 'pointer', fontFamily: FB, fontSize: 11, fontWeight: 600, textTransform: 'capitalize', background: statusF === s ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.05)', color: statusF === s ? W : WM, transition: 'all 0.2s' }}>
                ...
                >
                  <td style={{ padding: '14px 18px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <div style={{ width: 36, height: 36, borderRadius: '50%', background: `${ROLE_COLOR[u.role]}22`, border: `1px solid ${ROLE_COLOR[u.role]}44`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, color: ROLE_COLOR[u.role], fontWeight: 700, flexShrink: 0 }}>
                  ...
                  <td style={{ padding: '14px 18px' }}>
                    <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: ROLE_COLOR[u.role], background: `${ROLE_COLOR[u.role]}18`, padding: '3px 10px', borderRadius: 2 }}>{u.role}</span>
                  </td>
                  ...
                  <td style={{ padding: '14px 18px' }}>
                    <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: STATUS_COLOR[u.status], background: `${STATUS_COLOR[u.status]}18`, padding: '3px 10px', borderRadius: 2 }}>{u.status}</span>
                  </td>
        ...
        {(modal === 'create' || modal === 'edit') && (
          ...
          >
            <div style={{ background: '#141414', border: `1px solid ${BB}`, padding: '40px', width: '100%', maxWidth: 520, maxHeight: '90vh', overflowY: 'auto', position: 'relative' }}>
              ...
              <div style={{ fontSize: 10, letterSpacing: '0.3em', textTransform: 'uppercase', color: G, marginBottom: 8 }}>{modal === 'create' ? 'New User' : 'Edit User'}</div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                ...
                <button onClick={save} style={{ padding: '15px', background: G, border: 'none', color: B, fontFamily: FB, fontSize: 12, fontWeight: 700, letterSpacing: '0.15em', textTransform: 'uppercase', cursor: 'pointer', marginTop: 8, transition: 'background 0.2s' }}
                  onMouseEnter={e => e.currentTarget.style.background = GL}
                  onMouseLeave={e => e.currentTarget.style.background = G}
          ...
          >
            <div style={{ background: '#141414', border: `1px solid ${BB}`, padding: '40px', width: '100%', maxWidth: 460, position: 'relative' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 28 }}>
                <div style={{ width: 52, height: 52, borderRadius: '50%', background: `${ROLE_COLOR[editing.role]}22`, border: `1px solid ${ROLE_COLOR[editing.role]}44`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, color: ROLE_COLOR[editing.role], fontWeight: 700 }}>
                  {editing.name.split(' ').map(n => n[0]).join('').slice(0, 2)}
                </div>
                <div>
                  <div style={{ fontFamily: FD, fontSize: 22, fontWeight: 700, color: W }}>{editing.name}</div>
                  <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: ROLE_COLOR[editing.role], background: `${ROLE_COLOR[editing.role]}18`, padding: '3px 10px', borderRadius: 2 }}>{editing.role}</span>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 24 }}>
                <button onClick={() => { closeModal(); openEdit(editing) }} style={{ flex: 2, padding: '12px', background: G, border: 'none', color: B, fontFamily: FB, fontSize: 12, fontWeight: 700, cursor: 'pointer', transition: 'background 0.2s' }}
                  onMouseEnter={e => e.currentTarget.style.background = GL}
                  onMouseLeave={e => e.currentTarget.style.background = G}
                >Edit User</button>
                <button onClick={() => { closeModal(); setDeleting(editing.id) }} style={{ flex: 1, padding: '12px', background: 'rgba(239,68,68,0.15)', border: '1px solid #EF4444', color: '#EF4444', fontFamily: FB, fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>Delete</button>
              </div>
            </div>
          </div>
        )}
