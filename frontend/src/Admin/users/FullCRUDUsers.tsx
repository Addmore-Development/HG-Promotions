...
function mapApiUser(u: any, source: 'api' | 'local'): User {
  ...
  return {
    id:      u.id,
    name:    u.fullName || u.companyName || u.name || 'Unknown',
    email:   u.email || '',
...
const STATUS_BG: Record<Status, string> = {
  ...
  inactive:  hex2rgba('#6B4020', 0.35),
  suspended: hex2rgba(G4, 0.10),
  pending:   hex2rgba(GL, 0.10),
}
const STATUS_BORDER: Record<Status, string> = {
  active:    hex2rgba(G3, 0.45),
  inactive:  hex2rgba('#8B6040', 0.60),
  suspended: hex2rgba(G4, 0.42),
  pending:   hex2rgba(GL, 0.42),
}
...
export default function FullCRUDUsers() {
  ...
  const syncFromLocalStorage = () => {
    try {
      ...
      const localUsers = regs
        .filter((r: any) => r.role !== 'ADMIN')
        .map((r: any) => mapApiUser(r, 'local'))
    ...
    if (modal === 'create') {
      ...
      if (token) {
        fetch(`${API_URL}/users`, {
          method: 'POST',
          headers: { Authorization:`Bearer ${token}`, 'Content-Type':'application/json' },
          body: JSON.stringify({ fullName:form.name, email:form.email, phone:form.phone, city:form.city, role:form.role.toUpperCase(), status:form.status }),
        }).catch(() => {})
      }
    } else if (editing) {
  ...
  const counts = {
    ...
    api:       users.filter(u => u.source === 'api').length,
                      ...
                      {u.status === 'pending' && (
                        <div style={{ display:'flex', gap:6 }}>
                          <button onClick={() => updateUserStatus(u.id, 'active')}
                            style={{ flex:1, padding:'5px 10px', fontSize:10, fontWeight:700, color:B, background:`linear-gradient(135deg,${G3},${hex2rgba(G3,0.8)})`, border:`1px solid ${G3}`, borderRadius:3, cursor:'pointer', fontFamily:FD, transition:'all 0.15s', whiteSpace:'nowrap' }}
              ...
              <div style={{ display:'flex', alignItems:'center', gap:16, marginBottom:28 }}>
                ...
                <div>
                  ...
                  <div style={{ marginTop:6, display:'flex', gap:6, flexWrap:'wrap' }}>
                    ...
                    {editing.source && editing.source !== 'mock' && (
                      <span style={{ fontSize:9, fontWeight:700, color:editing.source==='api'?GL:G4, fontFamily:FD }}>
                        {editing.source === 'api' ? '● Live' : '◎ Local'}
                      </span>
                    )}
                  </div>
                </div>
              ...
              {editing.status === 'pending' && (
                <div style={{ display:'flex', gap:10, marginTop:20 }}>
                  <button onClick={() => { updateUserStatus(editing.id,'active'); closeModal() }}
                    style={{ flex:1, padding:'11px', background:hex2rgba(G3,0.18), border:`1px solid ${G3}`, color:G3, fontFamily:FD, fontSize:12, fontWeight:700, cursor:'pointer', borderRadius:3 }}>
                    ✓ Approve
                  </button>
                  <button onClick={() => { updateUserStatus(editing.id,'inactive'); closeModal() }}
                    style={{ flex:1, padding:'11px', background:hex2rgba(G2,0.18), border:`1px solid ${G2}`, color:'#C8A080', fontFamily:FD, fontSize:12, fontWeight:700, cursor:'pointer', borderRadius:3 }}>
                    ✗ Reject
                  </button>
                </div>
              )}
