  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16, marginBottom: 36 }}>
        ...
        <StatCard label="Active Jobs"      value={activeJobs.length}        sub="Currently live"      delay={60}  accent="#4ade80" />
        <StatCard label="Promoters Hired"  value={totalPromoters}           sub="Across all jobs"     delay={120} accent="#3A7BD5" />
                ...
                >
                  ...
                  <td style={{ padding: '16px 28px' }}>
                    <span style={{
                      ...
                      color: job.status === 'active' ? '#4ade80' : job.status === 'cancelled' ? '#ff6b6b' : WHITE_MUTED,
                      background: job.status === 'active' ? 'rgba(74,222,128,0.1)' : job.status === 'cancelled' ? 'rgba(255,107,107,0.1)' : 'rgba(255,255,255,0.05)',
                      padding: '4px 10px',
                    }}>
                      {job.status}
                    </span>
