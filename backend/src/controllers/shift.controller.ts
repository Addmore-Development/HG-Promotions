...
// ── Admin: Create a shift for a specific promoter ──────────────────────────
export const createShift = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { jobId, promoterId, status } = req.body;

    if (!jobId || !promoterId) {
      res.status(400).json({ error: 'jobId and promoterId are required' });
      return;
    }
    ...
    // Optional: prevent duplicate shift for same job + promoter?
    // (Uncomment if you want that check)
    // const existing = await prisma.shift.findFirst({
    //   where: { jobId, promoterId }
    // });
    // if (existing) {
    //   res.status(400).json({ error: 'Shift already exists for this promoter on this job' });
    //   return;
    // }
    ...
    console.error('[Shift] createShift error:', err);
...
export const deleteShift = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    // Optional: prevent deletion of non‑SCHEDULED shifts?
    // if (shift.status !== 'SCHEDULED') {
      //   return;
      //   res.status(400).json({ error: 'Only scheduled shifts can be deleted' });
    // }
